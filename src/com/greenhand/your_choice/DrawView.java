			...
			if (renren.isSessionKeyValid()==false) {
				renren.authorize(mainpage, new String[]{"status_update","photo_upload"},new RenrenAuthListener(){
					...
					public void onComplete(Bundle values) {
						...
						Toast.makeText(mainpage, "分享成功", Toast.LENGTH_SHORT).show();
					}
					@Override
					public void onRenrenAuthError(RenrenAuthError renrenAuthError) {
						Toast.makeText(mainpage, "登陆失败", Toast.LENGTH_SHORT).show();
					...
					public void onCancelLogin() {
						Toast.makeText(mainpage, "取消登陆", Toast.LENGTH_SHORT).show();
					}
					@Override
					public void onCancelAuth(Bundle values) {
						Toast.makeText(mainpage, "取消认证", Toast.LENGTH_SHORT).show();
					}
				});

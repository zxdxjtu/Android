...
class RenrenListenerFactory {
    ...
    private static final String WIDGET_FLAG_KEY = "flag";

    private static final int WIDGET_FLAG_SUCCESS = 1;

    private static final int WIDGET_FLAG_CANCEL = 2;

    private static final int WIDGET_FLAG_ERROR = 3;
        ...
        return new AbstractRenrenDialogListener() {
            ...
            public int onPageBegin(String url) {
                if (url.startsWith(Renren.WIDGET_CALLBACK_URI)) {
                    Bundle values = Util.parseUrl(url);
                    int flag = values.getInt(WIDGET_FLAG_KEY);
                    switch (flag) {
                        case 0://没有flag参数
                            if (values.size() > 1) {
                                listener.onComplete(values);
                            ...
                            break;
                        case WIDGET_FLAG_SUCCESS:
                            listener.onComplete(values);
                            break;
                        case WIDGET_FLAG_CANCEL:
                            listener.onCancel(values);
                            break;
                        case WIDGET_FLAG_ERROR:
                            listener.onError(values);
                            break;
                        default:
                            values.putString("desc", "Unknow flag " + flag);
                            listener.onError(values);
                    }
                    return RenrenDialogListener.ACTION_PROCCESSED;
                }
